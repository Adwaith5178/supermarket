...
const ProductCard = ({ product, onAddToCart, userRole }) => {
  ...
  }, [product.expiryDate, product.stockLevel]);
  ...
  return (
    <div className="product-card" style={{ position: 'relative', border: product.stockLevel < 5 && !isPurchased ? '1px solid #ff4757' : '1px solid #eee' }}>
      {/* 1. Global Alert Animations */}
      <style>
        {`
          @keyframes pulse-red {
      ...
      <div style={{ minHeight: '3.5rem', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
        {isPurchased ? (
          ...
          <>
            ...
            {isDealActive && (
              <div style={{ 
                background: product.stockLevel < 5 ? '#fff2f2' : '#f1f2f6', 
                padding: '6px 8px', 
                borderRadius: '4px', 
                marginTop: '4px',
                border: product.stockLevel < 5 ? '1px solid #ff4757' : 'none'
              }}>
                {product.stockLevel < 5 && product.stockLevel > 0 ? (
                  ...
                  <p style={{ color: '#ff4757', fontSize: '0.8rem', margin: 0, fontWeight: 'bold' }}>
                    ⏱ Ends in: {timeLeft}
                  </p>
                )}
              </div>
      ...
      <div style={{margin: '15px 0', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px'}}>
        <span style={{fontSize: '24px', fontWeight: 'bold', color: isPurchased ? '#888' : '#2ed573'}}>
          ₹{isDealActive ? product.currentPrice : product.basePrice}
        </span>
      ...
      {!isPurchased ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          <button onClick={() => onAddToCart(product)} style={styles.cartBtn} disabled={product.stockLevel <= 0}>
